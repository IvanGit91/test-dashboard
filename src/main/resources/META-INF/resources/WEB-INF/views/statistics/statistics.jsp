<link rel="stylesheet" href="views/statistics/statistics.css">

<div class="container subContainer" ng-controller="StatisticsCtrl as vm">
    <div class="loader-overlay" ng-show="!(semaforo == 0)">
        <div class="custom-loader" loader-css="ball-clip-rotate-multiple"></div>
    </div>
    <div class="row" ng-init="page.selected='quattro'">
            ...
            >
                <div ng-if="!template" class="ng-table-header"
                    ...
                    <span style="pointer-events: auto" ng-if="$column.title(this) === 'Test Name'"> <popup
                            position="top" datapopup="Test title performed" inverse="true"></popup> </span>
                    <span style="pointer-events: auto" ng-if="$column.title(this) === 'Attempts'"> <popup position="top"
                                                                                                          datapopup="It specifies how many times were perfomed each test"
                                                                                                          inverse="true"></popup> </span>
                    <span style="pointer-events: auto" ng-if="$column.title(this) === 'Success Rate'"> <popup
                            position="top" datapopup="It specifies the success rate for each test"
                            inverse="true"></popup> </span>
                    <span style="pointer-events: auto" ng-if="$column.title(this) === 'Performance (ms)'"> <popup
                            position="top" datapopup="It specifies the time (ms) taken to perform the zap action"
                            inverse="true"></popup> </span>
                    <span style="pointer-events: auto" ng-if="$column.title(this) === 'Min (ms)'"> <popup position="top"
                                                                                                          datapopup="It specifies the minimum value extracted on n tests performed"
                                                                                                          inverse="true"></popup> </span>
                    <span style="pointer-events: auto" ng-if="$column.title(this) === 'Max (ms)'"> <popup position="top"
                                                                                                          datapopup="It specifies the maximum value extracted on n tests performed"
                                                                                                          inverse="true"></popup> </span>
    ...
    <div class="row fast-fade" ng-show="isRendered">
        <div class="col-*-12">
            <!-- if remove the template-header element sorting is works -->
            <div class="dash-div">
                <button class="btn dash-btn" type="button" ng-click='downloadCsv(csv, $event, "all-statistics.csv")'
                        href=''>Download all statistics
                </button>
                <popup position="top"
                       datapopup="This button allows the user to download a csv file containing a list of all performance test reports"
                       inverse="true"></popup>
            </div>
            <div class="table-dashboard-container">
                <table ng-table-dynamic="tableParams with cols" class="table table-responsive table-dashboard"
                       show-filter="true" template-pagination="custom/pager" template-header="customHeader.html"
                       export-csv="csv" separator=";" export-csv-ignore="ignore">
